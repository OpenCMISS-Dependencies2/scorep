...
mirrors_merge( Cube* const newCube, const std::vector<const cube::Cube*>& cubes )
{
    ...
    {
        const std::vector<std::string>& __mirrors = ( *in_iter )->get_mirrors( false );
        for ( std::vector<std::string>::const_iterator iter1 = __mirrors.begin(); iter1 != __mirrors.end(); ++iter1 )
        {
            const std::vector<std::string>&          __already_mirrors = newCube->get_mirrors( false );
